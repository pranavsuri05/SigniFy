...
def draw_speech_bubble(frame, text, position, bubble_color=(255, 255, 255), text_color=(0, 0, 0)):
    ...
    # MASSIVELY increase font size
    font_scale = 3.5
    thickness = 8

    # Get text size dynamically
    (text_width, text_height), _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, font_scale, thickness)
    padding = 40  # More padding for a larger bubble
    w = text_width + padding
    h = text_height + padding

    # Adjust position to fit above the head
    bubble_x = x - w // 2
    bubble_y = y - h - 50  # Move bubble higher

    # Keep bubble inside frame boundaries
    bubble_x = max(20, min(bubble_x, frame.shape[1] - w - 20))
    bubble_y = max(20, bubble_y)

    # Draw BIGGER rounded rectangle (speech bubble)
...
while cap.isOpened():
    ...
    gesture_text = ""  # Default empty text
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            gesture_text = label_encoder[predicted_index]

    # Display the detected gesture as a "speech bubble" above head
    if gesture_text:
        h, w, _ = frame.shape
        bubble_position = (w // 2, 150)  # Higher positioning
        draw_speech_bubble(frame, gesture_text, bubble_position)
